    ...
    <div class="content-header">
      <div class="container-fluid">
        ...
        <div class="form-group has-feedback col-xs-6 col-md-6 col-lg-6 {{ $errors->has('discount') ? 'has-error' : '' }}">
            ...
            <input type="text" name="discount" class="form-control" value="{{old('discount',$product->offer_price)}}" >
        ...
        <div class="form-group">
              <div class="form-group has-feedback col-xs-6 col-md-6 col-lg-6 {{ $errors->has('category') ? 'has-error' : '' }}">
                  ...
                  @foreach($categories as $category)  
                     <option value="{{ $category->id }}" {{ $category->id == old('category_id', $product->category_id) ? 'selected' : '' }}>{{ $category->title }}</option>
                  @endforeach   
                   </select>
            </div>
      </div>
        ...
        <div class="form-group has-feedback col-xs-6 col-md-12 col-lg-12 {{ $errors->has('description') ? 'has-error' : '' }}">
            ...
            <textarea name="description" id="contant" rows="5" cols="50" style="width:60%" value="{{old('description')}}">{{$product->description}}</textarea>            
        ...
        <div class="form-group has-feedback col-xs-4 col-md-4 col-lg-4 {{ $errors->has('image') ? 'has-error' : '' }}"">
          ...
          @if($product && $product->image)
              <img src="../../uploads/products/{{ ($product->image) }}" class="img-thumbnail img-upload" id="blah" style="width: 280px">
          @else
          ...
          <div class="input-group">
            <span class="input-group-btn">
                <span class="btn btn-default btn-file" style="background-color: #06A8F0 ;color:#fff;">
                    Upload Image<input type="file" class="imgInp btn-file" name="image" id="imgInp" value="{{ old('image',$product->image) }}">
                </span>
            </span>
          </div>
          ...
          <div class="form-group has-feedback col-xs-4 col-md-4 col-lg-4 {{ $errors->has('bannerimage') ? 'has-error' : '' }}"">
           ...
           <label style="font-weight: 800" >Banner Image: </label>
            ...
            @if($product && $product->bannerimage)
              <img src="../../uploads/products/{{ $product->bannerimage }}" class="img-thumbnail img-upload" id="blahB" style="width: 280px">
            @else
        ...
        <div class="form-group" style="margin-left:5px; ">
            ...
            <div class="custom-control custom-radio">
                <input class="custom-control-input" type="radio" value="1" id="customRadio1" name="bestseller"  {{ $product->best_seller == '1' ? 'checked' : '' }}>
        ...
        <div class="form-group" style="margin-left:5px; ">
            ...
            <div class="custom-control custom-radio">
                <input class="custom-control-input" type="radio" value="1" id="customRadio3" name="showhomepage"  {{ $product->showhomepage == '1' ? 'checked' : '' }}>
