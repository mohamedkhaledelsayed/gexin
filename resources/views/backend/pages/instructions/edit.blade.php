...
<div class="content-wrapper">
  ...
  <form method="post" action="{{ route('instructionspage.update',$instructions->id) }}" enctype="multipart/form-data" >
                 @csrf
                 @method('PUT')
  <div class="card-body">
            ...
            <label>Title :</label>
            <input type="text" name="title" class="form-control"  value="{{ old('title',$instructions->title) }}">
            @if ($errors->has('title'))
        ...
        <div class="form-group has-feedback col-xs-6 col-md-12 col-lg-12 {{ $errors->has('description') ? 'has-error' : '' }}">
            ...
            <textarea name="description"  rows="5" cols="50" style="width:60%" value="{{old('description')}}">{{$instructions->description}}</textarea>            
